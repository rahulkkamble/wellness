...
/*
  ...
  - Single practitioner (from window.GlobalPractioner FHIR object) used as author/attester/performer
  - Generates a NDHM-friendly FHIR Bundle (Composition, Patient, Practitioner, Observations)
  - Additions:
    - Practitioner sourced from window.GlobalPractioner with safe fallbacks (id/name/license)
    - Axios POST to /api/v5/fhir-bundle with { bundle, patient: <originalPatientId> }
    - Patient and practitioner UI fields set to read-only (ABHA address selection remains enabled)
...
/* Map API patient to form shape (unchanged) */
...
/* Resolve practitioner from window.GlobalPractioner (FHIR) to {loginId,name,license} */
// function resolveGlobalPractitioner() {
//   const gp =
//     (typeof window !== "undefined" && (window.GlobalPractioner || window.GlobalPractionerFHIR)) ||
//     null;
...
function buildFhirPatient(form) {
  const id =
    form.resourceId && isUuid(form.resourceId) ? form.resourceId.toLowerCase() : uuidv4();
  const identifier = [];
...
function buildPractitioner(pract) {
  // const pract = window.GlobalPractioner;
  const id = uuidv4();
  return {
    ...
    identifier: [
      ...
      },
      {
        ...
        value: String(pract.loginId || "pract-001"),
        type: { coding: [{ system: V2_0203, code: "PN", display: "Person number" }], text: "Login id" },
      },
    ],
    name: [{ text: String(pract.name || "Dr. ABC") }],
  };

}

/* Observation builders (unchanged) */
...
function buildGeneralAssessmentObs(patientId, practitionerId, notes, pain) {
  ...
  return {
    ...
    status: "final",
    code: {
      coding: [{ system: "http://loinc.org", code: "8693-4", display: "Mental status" }],
...
function buildVitalObservation(patientId, practitionerId, codeText, quantity, unit, loincCode) {
  ...
  return {
    ...
    status: "final",
    code: loincCode
      ? { coding: [{ system: "http://loinc.org", code: loincCode, display: codeText }], text: codeText }
      : { text: codeText },
    subject: { reference: `urn:uuid:${patientId}` },
    performer: [{ reference: `urn:uuid:${practitionerId}` }],
    effectiveDateTime: new Date().toISOString(),
...
export default function App() {
  ...
  // Practitioner from global (read-only in UI)
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setApiPatients(apiData);
        const mapped = apiData.map(mapApiToForm);
        setForms(mapped);
        if (mapped.length > 0) {
          setSelectedIndex(0);
          setForm({
            ...mapped[0],
            selectedAbhaAddress:
              (mapped[0].abhaAddresses && mapped[0].abhaAddresses[0]) || mapped[0].abhaRef || "",
          });
        }
      } catch (apiErr) {
        ...
        try {
          const localRes = await fetch("/patients.json", { cache: "no-store" });
          const localData = await localRes.json();
          const arr = Array.isArray(localData) ? localData : localData?.data ?? [];
          setApiPatients(arr);
  ...
  useEffect(() => {
    // re-resolve practitioner in case global arrives late
    setPractitioner((prev) => {
      const r = resolveGlobalPractitioner();
      if (prev?.name !== r.name || prev?.license !== r.license || prev?.loginId !== r.loginId) return r;
      return prev;
    });
  }, []);
  ...
  useEffect(() => {
    if (selectedIndex >= 0 && forms[selectedIndex]) {
      ...
      setForm({
        ...f,
        selectedAbhaAddress: (f.abhaAddresses && f.abhaAddresses[0]) || f.abhaRef || "",
      });
    }
  }, [selectedIndex, forms]);
  ...
  async function generateBundle() {
    ...
    // vitals
    const hrObs = buildVitalObservation(
      patientRes.id,
      practitionerRes.id,
      "Heart rate",
      vitals.hr,
    ...
    const bpSys = buildVitalObservation(
      patientRes.id,
      practitionerRes.id,
    ...
    const bpDia = buildVitalObservation(
      patientRes.id,
      practitionerRes.id,
    ...
    if (vitals.spo2)
      observations.push(
        buildVitalObservation(patientRes.id, practitionerRes.id, "SpO2", vitals.spo2, "%", "59408-5")
      );

    // body
    if (body.height)
      observations.push(
        buildVitalObservation(patientRes.id, practitionerRes.id, "Height", body.height, "cm", "8302-2")
      );
    if (body.weight)
      observations.push(
        buildVitalObservation(patientRes.id, practitionerRes.id, "Weight", body.weight, "kg", "29463-7")
      );
    if (body.bmi)
      observations.push(
        buildVitalObservation(patientRes.id, practitionerRes.id, "BMI", body.bmi, "kg/m2", "39156-5")
      );

    // physical activity
    const physObs = buildPhysicalActivityObs(
      patientRes.id,
      practitionerRes.id,
      physicalText || "Not provided"
    );
    observations.push(physObs);

    // general assessment
    const genObs = buildGeneralAssessmentObs(
      patientRes.id,
      practitionerRes.id,
      generalNotes || "Not provided",
      !!generalPain
    );
    ...
    for (const li of lifestyle) {
      ...
      observations.push(
        buildLifestyleObs(patientRes.id, practitionerRes.id, li.label || "Lifestyle", li.value)
      );
    }
    ...
    const vitalsEntries = observations
    ...
    const lifestyleEntries = observations
      .filter((o) => (o.meta?.profile || []).includes(OBS_LIFESTYLE_PROFILE))
      .map((o) => ({ reference: `urn:uuid:${o.id}` }));
    ...
    try {
      ...
      console.error("Failed submission for patient:", bundle, originalPatientId);
    }
  ...
  return (
    <div className="container my-4">
      ...
      <p className="text-muted">
        Bootstrap UI. Select patient → choose ABHA address → edit → Generate FHIR Bundle
      </p>
            ...
            >
              ...
              {forms.map((f, i) => (
                <option key={i} value={i}>
                  {f.displayName || "Unknown"} — {f.gender || "unknown"} — {f.birthDate || "dob"}{" "}
                  {f.abhaRef ? `— ${f.abhaRef}` : ""}
                </option>
              ))}
            </select>
          ...
          {form && (
            <div className="row g-2">
              <div className="col-md-6">
                ...
                <input className="form-control" value={form.displayName} readOnly />
              </div>
              ...
              <div className="col-md-3">
                ...
                <input className="form-control" value={form.birthDate} readOnly />
              </div>

              <div className="col-md-4">
                <label className="form-label">Mobile</label>
                <input className="form-control" value={form.mobile} readOnly />
              </div>
              <div className="col-md-8">
                <label className="form-label">Address</label>
                <input className="form-control" value={form.address} readOnly />
              </div>

              <div className="col-md-6">
                <label className="form-label">ABHA Number</label>
                <input className="form-control" value={form.abhaRef} readOnly />
              </div>
              <div className="col-md-6">
                <label className="form-label">MRN</label>
                <input className="form-control" value={form.mrn} readOnly />
              </div>
              ...
              <div className="col-12">
                ...
                <select
                  className="form-select"
                  value={form.selectedAbhaAddress || ""}
                  onChange={(e) => updateFormField("selectedAbhaAddress", e.target.value)}
                >
                  <option value="">-- select ABHA address --</option>
                  {(form.abhaAddresses && form.abhaAddresses.length > 0
                    ? form.abhaAddresses
                    : form.abhaRef
                      ? [form.abhaRef]
                      : []
                ...
                <div className="form-text">
                  Select one ABHA address. No remove option for ABHA addresses.
                </div>
              </div>
            </div>
          )}
        ...
        <div className="card-body row g-2">
          ...
          <div className="col-12 mt-1">
            <small className="text-muted">
              Practitioner is included in bundle and used as author/attester/performer.
            </small>
          </div>
        </div>
      </div>
          ...
          <div className="row g-2">
            ...
            <div className="col-md-3">
              <input
                ...
                placeholder="BMI"
                value={body.bmi}
                onChange={(e) => setBody({ ...body, bmi: e.target.value })}
              />
            </div>
          </div>
        </div>
        ...
        <div className="card-body">
          ...
          <div className="form-check">
            <input
              ...
              type="checkbox"
              checked={generalPain}
              onChange={(e) => setGeneralPain(e.target.checked)}
            />
        ...
        <div className="card-body">
          {lifestyle.map((r, i) => (
            <div className="d-flex gap-2 align-items-center mb-2" key={i}>
              <input
                className="form-control"
                placeholder="Label (e.g. Smoking)"
                value={r.label}
                onChange={(e) => updateLifestyle(i, "label", e.target.value)}
              />
              <input
                className="form-control"
                placeholder="Value (true/false or text)"
                value={typeof r.value === "boolean" ? (r.value ? "true" : "false") : r.value}
                onChange={(e) => {
              ...
              <button
                ...
                disabled={lifestyle.length <= 1}
              >
                Remove
              </button>
            </div>
          ))}
          <button className="btn btn-primary" onClick={addLifestyle}>
            Add lifestyle
          </button>
        </div>
      </div>
      ...
      {/* {bundleJson && (
        <div id="bundlePreview" className="card mb-4">
          <div className="card-header d-flex justify-content-between align-items-center">
            ...
            <div>
              <button className="btn btn-sm btn-outline-primary" onClick={copyBundle}>
                Copy JSON
              </button>
            </div>
          </div>
